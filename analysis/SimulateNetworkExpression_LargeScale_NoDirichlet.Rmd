...
U.gam <- (6:(n.nei+1))		##Indices of unaffected neighbors; correspond to indices of Sigma
...
n.iter <- 2000
n.burn <- 1000
theta.gibbs <- c(5, 2, 2)  #If Dirichlet = T, this is the prior on alpha. Otherwise, this is proportional to the probability of unaffected, indirectly affected, directly affected by the cis-eQTL
Dirichlet <- FALSE
```
...
if (!sim.theta) {
  for (k in 1:n.theta) {
    ...
    for (i in 1:N) {
      ...
      data.plot.I[(n.I*(i-1)+1):(n.I*i)] <- apply(results.array[i,k,I.gam,], 1, sum)
      data.plot.U[(n.U*(i-1)+1):(n.U*i)] <- 1 - apply(results.array[i,k,U.gam,], 1, sum)
    }
    ...
    hist(data.plot.U, prob=T, xlim=c(0,1), xlab='Probability UNAFFECTED Simulated Gene is Affected by SNP', ylab='Density', main=paste0("eQTL Effect Size = ", tmp.theta, "x Expected Residual Size"))
    
    all.data <- c(1-data.plot.D, 1-data.plot.I, 1-data.plot.U); order.data <- order(all.data)
