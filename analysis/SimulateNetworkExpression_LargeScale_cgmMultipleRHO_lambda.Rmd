...
The purpose of this file is to test the performance of the Gibbs sampler with the modified Bayes factor, where I condition on the covariance matrix, $\Sigma$. I will simulate $N$ networks (i.e. covariance matrices), each with a corresponding effect size for the 'source' gene (i.e. the gene cis to the SNP), a list of indirect effects and a minor allele frequency. I will also shrink the indirect effects by $\lambda \in (0,1)$ and will use the model where I condition on $\Sigma$. For now, I do not attempt to learn $\lambda$ from the data, as the current method of using a $U(0,1)$ prior on $\lambda$ without updating the prior probabilities of an Indirect and Unaffected gene tend to produce too many false discoveries.
...
\[
 \mu_I = \theta x_{i} \Sigma_{12} \Sigma_{22}^{-1}\left[  \begin{matrix}
 1\\
 \vdots\\
 1\\
 0\\
 \vdots\\
 0
 \end{matrix}\right] \quad \text{where } \left[  \begin{matrix}
 1\\
 \vdots\\
 1\\
 0\\
 \vdots\\
 0
 \end{matrix}\right]^T 1 = |D|.
\]
The most important features of this simulation procedure are the minor allele frequency $f_i$, the (standardized) effect size $\theta_i$ and the correlation $\rho$ as these determine the power to detect effects over the noise in the data.
	...
	for (j in 1:N) {
	  ...
	  if (sim.theta) {
	    ...
	    for (theta.j in theta.try) {
	      Y.gex.j <- Sim.gex(Sigma.j, D.gam, I.gam, U.gam, theta.j, X.j, lambda=lambda)
